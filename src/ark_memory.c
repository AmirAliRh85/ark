...
{
    int len = ark_Vector_length(mem_manager->addressAllocated);

    printf("========== Info ==========\n");
    for (int i = 0 ; i < len ; i++)
    {
        ark_Object obj = *(ark_Object*)ark_Vector_at(mem_manager->addressAllocated , i);
...
ark_MemoryManager* ark_MemoryManager_create(char* log)
{
        ...
        mem_manager->addressAllocated = ark_Vector_create(sizeof(ark_Object));
...
void* ark_malloc(ark_MemoryManager* mem_manager , size_t _size , const char* _name)
{
    void* _memory = malloc(_size);
        ...
        else
        {
            ...
            ark_Vector_push(mem_manager->addressAllocated , &obj);
...
void ark_free(ark_MemoryManager* mem_manager , void* _memory)
{
        ...
        for (int i = 0 ; i < len ; i++)
        {
            obj = *(ark_Object*)ark_Vector_at(mem_manager->addressAllocated , i);
        ...
        {
            char buff[128];
            obj = *(ark_Object*)ark_Vector_at(mem_manager->addressAllocated , idx);
            ...
            ark_Vector_remove(mem_manager->addressAllocated , idx);
            free(_memory);
            ark_memDebug(mem_manager);
        }
    #else
...
void ark_MemoryManager_destroy(ark_MemoryManager* mem_manager)
{
            ...
            if (mem_manager->memAllocated > 0)
            {
                ...
                for (int i = len - 1 ; i >= 0 ; i--)
                {
                    ...
                    ark_Vector_pop(mem_manager->addressAllocated);
                }
        ...
        ark_Vector_destroy(mem_manager->addressAllocated);
        mem_manager->memAllocated = 0;
